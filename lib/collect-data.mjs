...
function collectOverview() {
  const firstCommitDate = git('log --reverse --format=%ad --date=short | head -1');
  const latestCommitDate = git('log -1 --format=%ad --date=short');
  const totalCommits = parseInt(git('rev-list --count HEAD'));
  const totalBranches = parseInt(git('branch -a | wc -l'));
  const totalTags = parseInt(git('tag | wc -l'));
...
function collectAIContributions() {
  ...
  for (const { name, authorPattern, allBranches } of aiPatterns) {
    ...
    const count = parseInt(git(`log ${branchFlag}--author="${authorPattern}" --oneline | wc -l`) || '0');
    if (count > 0) {
  ...
  const coAuthoredRaw = git('log --all --grep="Co-Authored-By" --oneline | wc -l');
  const coAuthored = parseInt(coAuthoredRaw || '0');
